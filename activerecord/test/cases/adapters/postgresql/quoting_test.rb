    ...
    class PostgreSQLAdapter
      class QuotingTest < ActiveRecord::TestCase
        ...
        def test_quote_range
          # There is no range support on 3.0's PG adapter, but we still want to
          ...
          sql = Bird.where(:name => range).to_sql
          expected_sql = %{SELECT "birds".* FROM "birds" WHERE ("birds"."name" BETWEEN '1,2]''; SELECT * FROM users; --' AND 'a')}
